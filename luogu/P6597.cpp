...
//	cout.tie(0);
	...
	cin >> n;
	newton(n - 2 + 1);
	mul(f, f, n - 2 + 1, n - 2 + 1, G);
	for(int i = 0; i * 2 < n - 2 + 1; ++i) G[i * 2] = (G[i * 2] + f[i]) % MOD;
	drep(i, n - 1, 1) G[i] = G[i - 1] * ksm(2, MOD - 2);
	G[0] = 0;
	G[n] = 0;
	mul(G, G, n + 1, n + 1, H);
	for(int i = 0; i * 2 < n + 1; ++i) H[i * 2] = (H[i * 2] + G[i]) % MOD;
	frep(i, 0, n + 1) H[i] = (H[i] * ksm(2, MOD - 2)) % MOD;
	rep(i, 2, n) cout << H[i] << endl;
    return 0;
}
