...
int fail[MAXN], tree[MAXN][26], id[MAXN], cnt[MAXN], endd;
char tmp[200][80], s[MAXN];
int ans[200];
...
int main(){
	while(true){
		...
		memset(id, 0, sizeof(int) * (endd + 5));
		memset(fail, 0, sizeof(int) * (endd + 5));
		memset(tree, 0, sizeof(tree[0]) * (endd + 5));
		endd = 0;
	}
	return 0;
}