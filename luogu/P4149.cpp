...
void finddis(int p, int fa, int dis, int dep){
	if(dep > ans || dis > m) return;
	ans = min(ans, dep + b[m - dis]);
	for(int i = head[p]; i != -1; i = nxt[i]){
		int x = to[i];
		if(d[i] == -1 || x == fa) continue;
		finddis(x, p, dis + d[i], dep + 1);
	}
	return;
}
void maintain(int p, int fa, int dis, int dep){
	if(dis > m) return;
	b[dis] = min(b[dis], dep);
	for(int i = head[p]; i != -1; i = nxt[i]){
		int x = to[i];
		if(d[i] == -1 || x == fa) continue;
		maintain(x, p, dis + d[i], dep + 1);
	}
	return;
}
